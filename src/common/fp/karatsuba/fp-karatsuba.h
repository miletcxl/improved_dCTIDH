#ifndef _FP_KARATSUBA_H_
#define _FP_KARATSUBA_H_
...
void  fp_add_s(fp c, const fp a, const fp b);	// a + a
...
void  fp_mont_redc(fp a, const uint64_t b[2 * NUMBER_OF_WORDS]);
...
#ifdef KARATSUBA
    // #if defined(P9215m85l389)
    // #define fp_mult_144x144 COMMON(fp_mult_144x144)
    // void  fp_mult_144x144(uint64_t *c, const uint64_t *a, const uint64_t *b);

    // #define mult_redc COMMON(mult_redc)
    // void  mult_redc(uint64_t *c, const uint64_t *a, const uint64_t *b);

    // #define add_redc COMMON(add_redc)
    // void  add_redc(uint64_t *c, const uint64_t *a, const uint64_t *b);    

    // #define fp_mont_it_redc COMMON(fp_mont_it_redc)
    // void  fp_mont_it_redc(fp a, const uint64_t b[2 * NUMBER_OF_WORDS]);  

    // #define add_redc_final COMMON(add_redc_final)
    // void  add_redc_final(uint64_t *c, const uint64_t *a, const uint64_t *b);   

    // #endif

    // #if defined(P8191m78l338) 
    ...
    // #define fp_mont_4k COMMON(fp_mont_4k)
    // void  fp_mont_4k(uint64_t *c, const uint64_t *a);

    // #define fp_word_redc COMMON(fp_word_redc)
    // void  fp_word_redc(uint64_t *c, const uint64_t *a);
    ...
    // #define fp_mont_2k COMMON(fp_mont_2k)
    // void  fp_mont_2k(uint64_t *c, const uint64_t *a);
