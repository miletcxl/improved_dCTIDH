...
//   const proj Pproj = {*P,fp_1};
  proj Aproj;
  fp_copy(Aproj.x, *A);
  fp_copy(Aproj.z, fp_1);
  proj Pproj;
  fp_copy(Pproj.x, *P);
  fp_copy(Pproj.z, fp_1);  
  ...
  for (long long i = primes_num - 1; i >= 0; --i)
    switch (validate_contribution(P1,A1,i)) {
      ...
      case 1:
        ...
        uintbig tmp;
        if (uintbig_sub(&tmp, &uintbig_four_sqrt_p, order))
...
static void test_validate(void)
{
  ...
  for (long long j = 0;j < 10;++j) {
    ...
    assert(!memcmp(&order0,&order2_slow,sizeof order0));
    uintbig tmp;
    assert(uintbig_sub(&tmp,&uintbig_four_sqrt_p,&order0));
  }
